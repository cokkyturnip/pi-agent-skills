...
**Pi Adaptation:** Original by awesome-skills. Language references loaded on-demand (progressive disclosure). Pi tools: use `read`/`bash`/`grep` equivalent to the original `Read`/`Grep`/`Bash`. Skip `allowed-tools` frontmatter — Pi auto-detects. The 4-phase process, severity labels, and all language guides apply directly.
...
| **Flutter / Dart** | [Flutter/Dart Guide](reference/flutter-dart.md) | Widgets, Dart 3, state management (BLoC/Riverpod/Provider/GetX/MobX/Signals), performance, accessibility, l10n, DI |
